...
const Greeting = () => <LanguageContext.Consumer>
    {contextValue => contextValue.translations.greeting}
  </LanguageContext.Consumer>;
const Headline = () => <LanguageContext.Consumer>
    {contextValue => contextValue.translations.headline}
  </LanguageContext.Consumer>;
const LanguageSelector = () => {
  return <LanguageContext.Consumer>
      {contextValue => <select onChange={event => {
      contextValue.changeLanguage(event.target.value);
    }}>
          {contextValue.availableLanguages.map(language => <option value={language}>{language}</option>)}
        </select>}
    </LanguageContext.Consumer>;
};
const ContextComponent = () => <Localized>
    <LanguageSelector />
    <p>
      <Greeting />
    </p>
...
export const Selector = () => {
  ...
  return <select onChange={event => {
    lang.changeLanguage(event.target.value);
  }}>
      {lang.availableLanguages.map(language => <option key={language} value={language}>{language}</option>)}
    </select>;
};
export const Greet = () => {
  const lang = useContext(LanguageContext);
  return <p>{lang.translations.greeting}</p>;
};
export const Head = () => {
  const lang = useContext(LanguageContext);
  return <p>{lang.translations.headline}</p>;
};