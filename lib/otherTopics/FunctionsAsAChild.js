...
export class FaaC extends React.Component {
  render() {
    return <div>
                ...
                <Formatter>{({
          bold
        }) => <p>This text {bold("does though")}.</p>}</Formatter>
                <Formatter children={({
        italic
      }) => <p>This text does know how to be {italic("italic")}.</p>} />
            </div>;
  }
}
const PriceTable = ({
...
class CryptoPrices extends React.Component {
  ...
  loadData = async () => {
    ...
    try {
      const cryptoTicker = await fetch('https://ed-6544990369349632.educative.run:3000' + '/api/v3/coins/markets?vs_currency=eur&per_page=' + limit.toString());
...
export default class FaaChild extends React.Component {
  render() {
    return <CryptoPrices limit={5}> {props => <PriceTable {...props} />} </CryptoPrices>;
  }
}