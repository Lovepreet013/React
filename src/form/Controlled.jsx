        ...
        return(<form method="post" onSubmit={this.submitForm}>
          ...
          <p>
            <input
              ...
              value={this.state.username} //can only be string, null and undefined are not allowed
            />
...
export class ControlledComponents extends React.Component {
    state = {
        text: '', //if using this destructuring method, the state variable must be same as the name attribute of the input else it will not work and it creates the new state variable with the name of the name attribute of the input
        textarea: '',
        checkbox: false,
        singleSelect: '',
        multipleSelect: [],
    ...
    //destructuring and getting target propety from event object and then getting name and value from target. Input must have name and value attribute for this to work
    ...
    render(){
        return(
            ...
            <input
              ...
              onClick={this.renderEventTarget}
            />
            <br/><br/>
            <h2>Text Area</h2>
            <textarea
            ...
            {this.state.notes.map((note) => (
                <div key={note.id} style={{display : "flex", alignItems : "center"}}>
                    ...
                    <p style={{textDecoration : note.checked ? "line-through" : "none" }}>{note.value}</p>
